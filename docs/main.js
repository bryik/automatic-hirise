...
async function createTerrainMesh(planeGeometryOptions, terrainElevations) {
  let geometry = new THREE.PlaneBufferGeometry(...Object.values(planeGeometryOptions));
  ...
  geometry.addAttribute('elevation', new THREE.BufferAttribute(terrainElevations, 1));

  let uniforms = {
    u_verticalExaggeration: { type: 'f', value: 25.0 }
  };

  let vertexShader = await fetch(vertexShaderPath).then(response => response.text());
  let fragmentShader = await fetch(fragmentShaderPath).then(response => response.text());
